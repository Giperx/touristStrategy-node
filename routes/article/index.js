...
const {
    ...
    modelComment,
    modelConcern
} = require('@/models/collection') // 引入数据模型
const { // 引入数据校验
    Articlekeyword,
    Articlepblish,
    ArticlepblishVideo,
...
router.post('/article-keyword', async ctx => {
    // console.log('11');
    // await aiKeyword('我喜欢南昌，我要去青山湖公园玩！！')
...
// 小程序端：发布图片类型的游记，通过权限接口校验token
router.post('/article-publish', new Auth().m, async ctx => {
    // imageInfor('https://wx3.sinaimg.cn/mw690/005XpTbZly8hnwimrrl12j30rs0rdq3k.jpg')
...
router.get('/comments-data', new Authuid().m, async ctx => {
    ...
    const res = await modelComment.aggregate([
        // { $match: { article_id: new mongoose.Schema.Types.ObjectId(article_id) } },
